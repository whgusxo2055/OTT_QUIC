...
| FR-AUTH-04 | 비밀번호는 해시화하여 저장한다 (bcrypt 사용) | 필수 |
| FR-AUTH-05 | 로그인 실패 시 에러 메시지를 표시한다 | 필수 |
| FR-AUTH-06 | 세션 정보는 SQLite에 저장되어 재시작 후에도 복원된다 | 필수 |
| FR-PLAY-05 | QUIC 기반 스트리밍은 WebSocket 브리지로 브라우저에 전달되며 이어보기를 지원한다 | 필수 |
